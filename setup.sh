...
count=$(cd ${epgimport}/ && ls -d *echannelizer.*.sources.xml* | wc -l)
if [ $count -eq "1" ]; then
   filename=$(find ${epgimport}/ -name "echannelizer.*.sources.xml" -exec basename {} \;)
   echo "One echannelizer playlist found: $filename, proceeding installation..."
   sleep 2
else
echo "Enter full file name from below list starting with echannelizer.XXXX.sources.xml, For example: You want to fix EPG offset for provider's playlist named IPTV in echannelizerxtreme then you should look for file named echannelizer.IPTV.sources.xml"
echo
find $epgimport/ -name "echannelizer.*.sources.xml" -exec basename {} \;
echo
